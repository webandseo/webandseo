# Mission — cloisonner 23 sites o2switch sur 4 lunes (compte `trlh2564`)

## L'objectif

23 sites cohabitent dans le compte cPanel `trlh2564`. Une seule compromission
les expose tous. Quatre lunes (sous-comptes cPanel isolés) ont été créées pour
cloisonner.

**19 sites sont à déplacer.** Les 4 autres restent où ils sont.

C'est la troisième fois : les comptes `webandseo` et `qvle6290` sont déjà
migrés, avec le même outillage. Et c'est le plus simple des trois — **aucun
sous-domaine, aucune boîte email, aucun cas particulier.** 19 sites ordinaires,
la même séquence répétée dix-neuf fois.

## Répartition à appliquer

| Compte cible | Sites | Action |
|---|---|---|
| `trlh2564` *(principal)* | `revue-de-synthese.eu`, `vv-artdesign.com`, `wepeek.fr`, `yatoo.org` | **aucune** — ils y sont déjà |
| `sc1trlh2564` | `aliens-cafe.com`, `artswall.fr`, `blogvoyage.eu`, `caet.fr`, `delsoko.fr` | 5 migrations |
| `sc2trlh2564` | `espace-zen.fr`, `ftpix.fr`, `guidevoyage.net`, `herve-sarl.fr`, `kalaphoto.fr` | 5 migrations |
| `sc3trlh2564` | `laiton.eu`, `le-paysagiste.net`, `leblogdubienetre.com`, `morningcoffee.fr` | 4 migrations |
| `sc4trlh2564` | `muxi.fr`, `paiecheck.com`, `plomb.eu`, `programme-repere.fr`, `publiepapier.fr` | 5 migrations |
...
complète `plan/trlh2564/inventaire.csv` avec les tailles : c'est ce qui rend
...
disque à disque, en local. Lance donc les scripts **en SSH sur `trlh2564`**,
jamais depuis un poste distant.

## Prérequis, une seule fois

1. **Autorisation SSH** : cPanel → `Sécurité` → `Autorisation SSH`, déclarer ton
   IP. Sur `trlh2564` **et** sur les quatre lunes.
2. **Clé SSH de `trlh2564` vers chaque lune.** Depuis `trlh2564` :
...
4. **Vérifier** :
       ...
       ./o2s-migrer.sh --domaine aliens-cafe.com --dst-user sc1trlh2564 --phase check

   Si la connexion échoue alors que la clé est autorisée, le shell cloisonné
   n'accepte peut-être pas `localhost` : teste `ssh sc1trlh2564@<serveur>` avec
...
comptes.**
    ...
    ./o2s-plan.sh --csv ../plan/trlh2564/inventaire.csv                     # les 19
    ./o2s-plan.sh --csv ../plan/trlh2564/inventaire.csv --lune sc1trlh2564  # une lune
...
base, `wp-config.php` recâblé. Le domaine pointe toujours sur `trlh2564` : rien
...
1. `trlh2564` → `Domaines` → SITE → **Supprimer**
...
**Puis, sur la lune**
    ...
    ./o2s-migrer.sh --domaine SITE --dst-user LUNE --src-user trlh2564 \
...
Une fois les 19 sites déplacés, vérifie qu'aucun certificat n'est passé à la
trappe. Depuis n'importe quel compte :

    for d in $(awk -F, 'NR>1 && $14 ~ /^sc/ {print $1}' ../plan/trlh2564/inventaire.csv); do
      printf '%-32s ' "$d"
...
Les 19 lignes doivent afficher un émetteur Let's Encrypt et une date
...
qu'aucun de ces 23 domaines n'a de sous-domaine ni de boîte email. Si
...
- **Ne supprime jamais rien sur `trlh2564`.** Les scripts ne le font pas ; ne le
  fais pas non plus. Les fichiers et bases y restent intacts après la bascule :
  c'est le filet.
- **Retour arrière** : retirer le domaine de la lune, le recréer sur `trlh2564`
...
- **Ne fais pas le ménage sur `trlh2564`** avant plusieurs jours de
  fonctionnement normal, tâches cron vérifiées.
- `--reecrire-chemins` exige `--src-user trlh2564`. Ne le retire pas.
...
2. Ensuite les 18 autres, du plus léger au plus lourd. 15 à 20 minutes chacun
   une fois la procédure rodée.
3. Les plus visités en dernier, à heure creuse.

N'enchaîne pas les 19 d'un bloc. Chaque bascule mérite ses quinze minutes
d'attention.
...
- **Le contrôle SSL de fin de campagne**, les 19 lignes
- Un point global une fois les 19 faits, avec ce qui reste à finir
...
- `o2switch/plan/trlh2564/repartition.md` — la répartition de ce compte
